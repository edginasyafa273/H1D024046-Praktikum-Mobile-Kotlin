# Laporan Praktikum Pemrograman Mobile

**Nama**     : Edgina Syafa Ayu Wicaksono
**NIM**      : H1D024046
**Shift**    : Awal E / Akhir E
**Praktikum**: Pemrograman Mobile

## Tugas Pertemuan 1
**Tanggal**: Selasa, 1 September 2026

## Screenshot
<img width="250" alt="WhatsApp Image 2026-09-08 at 19 56 05" src="https://github.com/user-attachments/assets/70effcea-7b28-4113-92e3-68f740408305" />

<img width="250" alt="WhatsApp Image 2026-09-08 at 19 45 17" src="https://github.com/user-attachments/assets/862e3303-b67e-49e9-ade2-2a6e7462bbde" />

**Kesimpulan Praktikum:**  
Pada pertemuan pertama, praktikum memberikan pemahaman dasar mengenai inisialisasi proyek Android dan penerapan konsep Declarative UI dengan Jetpack Compose. Mahasiswa mempelajari struktur dasar proyek, komponen layout fundamental seperti Column, Row, dan Box, serta cara membangun tampilan sederhana menggunakan fungsi `@Composable` tanpa memerlukan file XML terpisah.

## Tugas Pertemuan 2

## Screenshot
<img width="738" height="1600" alt="WhatsApp Image 2026-09-16 at 00 10 43 (1)" src="https://github.com/user-attachments/assets/e23b84d4-f6f7-42de-947f-7cdd6750baeb" />
<img width="738" height="1600" alt="WhatsApp Image 2026-09-16 at 00 10 43" src="https://github.com/user-attachments/assets/31ad5bac-504e-48eb-97c3-0e2b2d99c28d" />


**Kesimpulan Praktikum:**  
Pertemuan kedua menekankan pada implementasi tema visual yang konsisten dan fitur interaktif dalam aplikasi mobile. Mahasiswa belajar bagaimana memisahkan konfigurasi desain (warna, tipografi, bentuk) dari logika komponen, serta menghubungkan antarmuka dengan logika program menggunakan state dan navigasi antar halaman sehingga aplikasi dapat merespons input pengguna secara dinamis.

## Tugas Pertemuan 3

## Screenshot
<img width="738" height="1600" alt="WhatsApp Image 2026-09-22 at 17 09 26" src="https://github.com/user-attachments/assets/e3998b58-3030-43f5-99d7-1542ccef45d1" />
<img width="738" height="1600" alt="WhatsApp Image 2026-09-22 at 17 09 26 (1)" src="https://github.com/user-attachments/assets/0e778274-34fa-4c9f-acac-3fdb4970e6a4" />



**Kesimpulan Praktikum:**  
Pertemuan ketiga membuka wawasan tentang pengembangan aplikasi mobile yang lebih kompleks dengan data dinamis. Mahasiswa belajar bagaimana mengintegrasikan struktur data terstruktur (data class) dengan komponen tata letak yang efisien (Lazy Layouts) untuk menciptakan aplikasi yang mampu menampilkan dan menyaring data dalam jumlah besar tanpa mengorbankan performa aplikasi.
